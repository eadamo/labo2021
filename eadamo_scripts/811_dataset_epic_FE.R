...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , EZE_Fvencimiento        := pmax( Master_Fvencimiento, Visa_Fvencimiento, na.rm = TRUE) ]
  dataset[ , EZE_Finiciomora         := pmax( Master_Finiciomora, Visa_Finiciomora, na.rm = TRUE) ]
  ...
  dataset[ , EZE_fultimo_cierre      := pmmin( Master_fultimo_cierre, Visa_fultimo_cierre, na.rm = TRUE) ]
  dataset[ , mv_mpagado              := rowSums( cbind( Master_mpagado,  Visa_mpagado) , na.rm=TRUE ) ]
  dataset[ , mv_mpagospesos          := rowSums( cbind( Master_mpagospesos,  Visa_mpagospesos) , na.rm=TRUE ) ]
  dataset[ , mv_mpagosdolares        := rowSums( cbind( Master_mpagosdolares,  Visa_mpagosdolares) , na.rm=TRUE ) ]
  dataset[ , mv_fechaalta            := pmax( Master_fechaalta, Visa_fechaalta, na.rm = TRUE) ]
  dataset[ , EZE_fechaalta           := pmin( Master_fechaalta, Visa_fechaalta, na.rm = TRUE) ]
  ...
  dataset[ , EZE_mprestamos                        := rowSums( cbind(mprestamos_personales, mprestamos_prendarios, mprestamos_hipotecarios) , na.rm=TRUE ) ]
  dataset[ , EZE_cprestamos                        := rowSums( cbind(cprestamos_personales, cprestamos_prendarios, cprestamos_hipotecarios) , na.rm=TRUE ) ]
  dataset[ , EZE_cseguros                          := rowSums( cbind(cseguro_vida, cseguro_auto, cseguro_vivienda,cseguro_accidentes_personales ) , na.rm=TRUE ) ]
  
  dataset[ , EZE_ctarjeta_cuentas                  := rowSums( cbind(ctarjeta_master, ctarjeta_visa ) , na.rm=TRUE ) ]
  dataset[ , EZE_ctarjeta_transacciones            := rowSums( cbind(ctarjeta_master_transacciones , ctarjeta_visa_transacciones ) , na.rm=TRUE ) ]
  dataset[ , EZE_mtarjeta_consumos                 := rowSums( cbind(mtarjeta_master_consumo , mtarjeta_visa_consumo ) , na.rm=TRUE ) ]
  dataset[ , EZE_cdebitos_automaticos              := rowSums( cbind(ctarjeta_master_debitos_automaticos  , ctarjeta_visa_debitos_automaticos ) , na.rm=TRUE ) ]
  dataset[ , EZE_mdebitos_automaticos              := rowSums( cbind(mttarjeta_master_debitos_automaticos , mtarjeta_visa_debitos_automaticos ) , na.rm=TRUE ) ]
  dataset[ , EZE_cdescuentos                       := rowSums( cbind(ctarjeta_master_descuentos, ctarjeta_visa_descuentos) , na.rm=TRUE ) ]
  dataset[ , EZE_mdescuentos                       := rowSums( cbind(mtarjeta_master_descuentos, mtarjeta_visa_descuentos) , na.rm=TRUE ) ]
  dataset[ , EZE_delinquency                       := pmax( Master_delinquency, Visa_delinquency, na.rm = TRUE) ]
  
  dataset[ , EZE_mcajas_ahorro                     := rowSums( cbind( mcaja_ahorro, mcaja_ahorro_adicional,mcaja_ahorro_dolares) , na.rm=TRUE ) ]
  dataset[ , EZE_mcuenta_corrientes                := rowSums( cbind( mcuenta_corriente,  mcuenta_corriente_adicional) , na.rm=TRUE ) ]
  dataset[ , EZE_r_mcuenta_corrientes_descubierto  := mdescubierto_preacordado  / EZE_mcuenta_corrientes ]
  dataset[ , EZE_r_ccheques_rechazados_depositados := ccheques_depositados_rechazados  / ccheques_depositados ]
  dataset[ , EZE_r_mcheques_rechazados_depositados := mcheques_depositados_rechazados  / mcheques_depositados ]
  dataset[ , EZE_r_ccheques_rechazados_emitidos    := ccheques_emitidos_rechazados  / ccheques_emitidos ]
  dataset[ , EZE_r_mcheques_rechazados_emitidos    := mcheques_emitidos_rechazados  / mcheques_emitidos ]
  dataset[ , EZE_r_antiguedad_edad                 := (cliente_antiguedad / 12)  / cliente_edad ]
  dataset[ , EZE_r_mpagominimo_consumo             := mv_mpagominimo  / mv_mconsumototal ]
  dataset[ , EZE_r_mcuentas_saldo_edad             := mcuentas_saldo  / cliente_edad ]
  dataset[ , EZE_r_mcomisiones                     := mcomisiones  / cliente_antiguedad ]
